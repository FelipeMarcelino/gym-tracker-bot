"""Unit tests for core services with working APIs"""
...
from services.async_health_service import BotMetrics, HealthService, HealthStatus, SystemMetrics
...
class TestBackupServiceCore:
    """Test core backup service functionality"""
    ...
    def test_backup_service_automation(self):
        """Test backup automation controls"""
        with tempfile.TemporaryDirectory() as temp_dir:
...
class TestHealthServiceCore:
    """Test core health service functionality"""
    ...
    def test_health_service_metrics_recording(self):
...
class TestShutdownServiceCore:
    """Test core shutdown service functionality"""

    def test_shutdown_service_initialization(self):
    ...
    def test_handler_registration_duplicate(self):
        ...
        service.register_shutdown_handler(test_handler, "Test handler")
        service.register_shutdown_handler(test_handler, "Test handler again")
...
class TestServiceErrorHandling:
    ...
    async def test_backup_service_resilience(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            service.database_path = "/invalid/path/database.db"
...
class TestServiceConfiguration:
    ...
    def test_shutdown_service_configuration(self):
        """Test shutdown service accepts various configurations"""
        configs = [
            {"shutdown_timeout": 10},
            {"shutdown_timeout": 30},
            {"shutdown_timeout": 60},
        ...
        for config in configs:
            service = ShutdownService(**config)
            assert service.shutdown_timeout == config["shutdown_timeout"]

    def test_health_service_configuration(self):
        """Test health service works with different configurations"""
        # Health service should work regardless of environment
        service = HealthService()
