...
class TestBackupService:
    ...
    async def test_backup_verification(self, test_backup_service, sync_populated_test_database):
        ...
        # Create valid backup
        backup_path = await test_backup_service.create_backup("valid_backup.db")
    ...
    async def test_list_backups(self, test_backup_service, sync_populated_test_database):
        ...
        backup1 = await test_backup_service.create_backup("backup1.db")
        time.sleep(0.1)  # Ensure different timestamps
        backup2 = await test_backup_service.create_backup("backup2.db")
    ...
    async def test_backup_stats(self, test_backup_service, sync_populated_test_database):
        ...
        # Create backups
        await test_backup_service.create_backup("backup1.db")
        time.sleep(0.1)
        await test_backup_service.create_backup("backup2.db")
    ...
    async def test_cleanup_old_backups(self, test_backup_service, sync_populated_test_database):
        ...
        for i in range(5):
            await test_backup_service.create_backup(f"backup{i}.db")
            time.sleep(0.1)
        ...
        await test_backup_service.cleanup_old_backups()
    ...
    async def test_restore_backup_validation(self, test_backup_service):
        ...
        with pytest.raises(BackupError) as exc_info:
            await test_backup_service.restore_backup("some_backup.db", confirm=False)
    ...
    async def test_error_handling(self, test_backup_service):
        ...
        with pytest.raises(BackupError):
            await test_backup_service.create_backup("test.db")
...
class TestBackupServiceEdgeCases:
    ...
    async def test_backup_name_generation(self, test_backup_service, sync_populated_test_database):
        ...
        # Without custom name
        backup_path = await test_backup_service.create_backup()
    ...
    async def test_concurrent_backup_operations(self, test_backup_service, sync_populated_test_database):
        ...
        backup1 = await test_backup_service.create_backup("concurrent1.db")
        backup2 = await test_backup_service.create_backup("concurrent2.db")
    ...
    async def test_backup_with_special_characters(self, test_backup_service, sync_populated_test_database):
        ...
        backup_path = await test_backup_service.create_backup("backup-with_special.chars.db")
        assert os.path.exists(backup_path)
