...
class Settings(BaseSettings):
    ...
    SESSION_TIMEOUT_HOURS: int = Field(default=3, gt=0, le=24, description="Session timeout in hours")

    # Rate limiting settings
    RATE_LIMIT_GENERAL_REQUESTS: int = Field(default=20, gt=0, le=1000, description="General request rate limit")
    ...
    MAX_VOICE_FILE_SIZE_MB: int = Field(default=100, gt=0, le=500, description="Max voice file size in MB")

    # Duration limits (in seconds)
    MAX_AUDIO_DURATION_SECONDS: int = Field(default=300, gt=0, le=3600, description="Max audio duration in seconds")

    # Text limits
    MAX_TEXT_LENGTH: int = Field(default=1000, gt=0, le=50000, description="Max text length")
    MAX_NAME_LENGTH: int = Field(default=100, gt=0, le=500, description="Max name length")
    MAX_TRANSCRIPTION_LENGTH: int = Field(default=10000, gt=0, le=100000, description="Max transcription length")

    # LLM settings
    LLM_TEMPERATURE: float = Field(default=0.1, ge=0.0, le=2.0, description="LLM temperature")
    LLM_MAX_TOKENS: int = Field(default=8000, gt=0, le=100000, description="LLM max tokens")

    # Logging limits
    LOG_TEXT_PREVIEW_LENGTH: int = Field(default=100, gt=0, le=1000, description="Log text preview length")
    LOG_MESSAGE_PREVIEW_LENGTH: int = Field(default=50, gt=0, le=500, description="Log message preview length")
