...
async def _process_workout_audio_optimized(
    ...
    try:
        ...
        if hasattr(e, 'user_message') and e.user_message:
            ...
            error_msg = messages.ERROR_VALIDATION.format(
...
async def _process_workout_message(
    ...
    session_manager = await get_async_session_manager()
    workout_session, is_new = await session_manager.get_or_create_session(
        user_id
    )
    ...
    status_msg = await update.message.reply_text(
        initial_msg, parse_mode='Markdown'
    )
...
async def finish_command(
    ...
    result = await workout_service.finish_session(
        last_session.session_id, user_id
    )
...
async def stats_command(
    ...
    try:
        ...
        # Get analytics (async)
        analytics = await analytics_service.get_workout_analytics(
            user_id, days=days
        )
...
async def progress_command(
    ...
    try:
        ...
        progress = await analytics_service.get_exercise_progress(
        ...
        logger.info(
            f'Progresso calculado: {exercise_name} para usuário {user_id}'
        )
    ...
    try:
        ...
        async with get_async_session_context() as session:
            ...
            if resistance_exercises:
                message += (
                    f'💪 **Resistência ({len(resistance_exercises)}):**\n'
                )
        ...
        logger.info(
            f'Lista de exercícios enviada: {len(exercises)} exercícios'
        )
...
def _format_analytics_message(
    analytics: Dict[str, Any], user_name: str
) -> str:
    ...
    message += (
        f"✅ Taxa de conclusão: {session_stats['completion_rate']:.1f}%\n"
    )
    ...
    if frequency['longest_streak_days'] > 1:
        message += (
            f"🔥 Maior sequência: {frequency['longest_streak_days']} dias\n"
        )
    message += '\n'
...
def _format_progress_message(progress: Dict[str, Any]) -> str:
    ...
    if history:
        ...
        for session in recent_sessions:
            ...
            if weights and reps:
                sets_info = ', '.join(
                    [f'{r}×{w}kg' for r, w in zip(reps, weights)]
                )
...
async def ratelimit_stats_command(
    ...
    try:
        ...
        from services.rate_limit_cleanup_service import (
            rate_limit_cleanup_service,
        )
        ...
        message += (
            f"• Frequência: {cleanup_stats['cleanup_frequency_hours']}h\n"
        )
        message += (
            f"• Inatividade máxima: {cleanup_stats['max_inactive_seconds']}s\n"
        )
