...
class ValidationRule(BaseModel):
    ...
    required: bool = Field(
        default=True, description='Whether the field is required'
    )
...
class TextValidator(BaseValidator):
    ...
    def validate(
        ...
        if not text and self.allow_empty:
            return {'is_valid': True, 'value': text, 'error': None}
...
class NumberValidator(BaseValidator):
    ...
    def validate(
        ...
        return {'is_valid': True, 'value': num, 'error': None}
...
class AudioValidator(BaseValidator):
    ...
    def validate(
        ...
        try:
            if (
                hasattr(value, 'duration')
                and isinstance(value.duration, (int, float))
                and value.duration > self.max_duration
            ):
                return {
                    'is_valid': False,
                    'value': value,
        ...
        try:
            if (
                hasattr(value, 'file_size')
                and isinstance(value.file_size, (int, float))
                and value.file_size > max_size_bytes
            ):
                return {
                    'is_valid': False,
                    'value': value,
        ...
        return {'is_valid': True, 'value': value, 'error': None}
...
class CommandArgsValidator(BaseValidator):
    ...
    def validate(
        ...
        return {'is_valid': True, 'value': validated_args, 'error': None}
    ...
    def validate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.fields:
            ...
            if self.audio_validator and 'audio' in data:
                validation_result = self.audio_validator.validate(
                    data['audio']
                )
                if validation_result['is_valid']:
...
class ValidationMiddleware:
    ...
    async def validate_update(
        ...
        if update.effective_user:
            user_validation = ValidationMiddleware._validate_user(
            ...
            for field, validator in schema.custom_validators.items():
                ...
                if not validation_result['is_valid']:
                    ...
                    result['errors'].append(
                        f"{field}: {validation_result['error']}"
                    )
                else:
                    result['data']['custom'][field] = validation_result[
                        'value'
                    ]
    ...
    def decorator(func):
        ...
        async def wrapper(
            ...
            try:
                ...
                logger.exception(
                    f'Unexpected error in validation decorator: {e}'
                )
                raise
    ...
    def voice_message(max_duration: Optional[int] = None) -> ValidationSchema:
        ...
        return ValidationSchema(
            ...
            audio_validator=AudioValidator(
                ...
                or settings.MAX_AUDIO_DURATION_SECONDS
            ),
        )
