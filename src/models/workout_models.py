...
class ResistanceExercise(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description='Exercise name'
    )
...
class AerobicExercise(BaseModel):
    ...
    intensity_level: Optional[
        Literal['low', 'moderate', 'high', 'hiit']
    ] = Field(None, description='Exercise intensity')
    notes: Optional[str] = Field(
        None, max_length=500, description='Additional notes'
    )
...
class WorkoutData(BaseModel):
    ...
    notes: Optional[str] = Field(
        None, max_length=1000, description='Session notes'
    )
...
class LLMParseResult(BaseModel):
    ...
    workout_data: Optional[WorkoutData] = Field(
        None, description='Parsed workout data'
    )
    ...
    def validate_result_consistency(self):
        ...
        if self.success and self.workout_data is None:
            raise ValueError(
                'Success=True requires workout_data to be present'
            )

        if not self.success and not self.errors:
            raise ValueError(
                'Success=False requires at least one error message'
            )
