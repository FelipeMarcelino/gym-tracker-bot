...
class ActiveUsersCount(BaseModel):
    ...
    general: int = Field(
        ge=0, description='Active users in general rate limiter'
    )
    voice: int = Field(ge=0, description='Active users in voice rate limiter')
    commands: int = Field(
        ge=0, description='Active users in commands rate limiter'
    )
...
class RateLimiterStats(BaseModel):
    ...
    limits: dict[str, RateLimitConfig] = Field(
...
class CleanupResult(BaseModel):
    ...
    general: int = Field(
        ge=0, description='Users cleaned from general limiter'
    )
    voice: int = Field(ge=0, description='Users cleaned from voice limiter')
    commands: int = Field(
        ge=0, description='Users cleaned from commands limiter'
    )
...
class ErrorContext(BaseModel):
    ...
    value: Optional[str] = Field(
        default=None, description='Value that was invalid'
    )
    ...
    backup_path: Optional[str] = Field(
        default=None, description='Backup file path'
    )
...
class ExportSummary(BaseModel):
    ...
    completed_sessions: int = Field(
        ge=0, description='Number of completed sessions'
    )
    ...
    aerobic_exercises: int = Field(
        ge=0, description='Number of aerobic exercises'
    )
...
class ExportResult(BaseModel):
    ...
    message: Optional[str] = Field(
        default=None, description='Error or info message'
    )
...
class ExportPreview(BaseModel):
    ...
    estimated_size_mb: float = Field(
        ge=0, description='Estimated export size in MB'
    )
