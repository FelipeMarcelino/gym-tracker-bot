...
class AsyncAnalyticsService:
    ...
    async def get_exercise_progress(
        ...
        async with get_async_session_context() as session:
            try:
                ...
                progress_stmt = select(WorkoutExercise).join(WorkoutSession).where(
                    WorkoutSession.user_id == user_id,
    ...
    async def _calculate_comprehensive_analytics(
        ...
        # Workout frequency
        workout_frequency = await self._calculate_workout_frequency(sessions, days)
    ...
    async def _calculate_workout_frequency(self, sessions: List[WorkoutSession], days: int) -> Dict[str, Any]:
        ...
        if len(dates) > 1:
            ...
            if ideal_gap > 0:
                consistency_score = max(0, 1 - abs(avg_gap - ideal_gap) / ideal_gap)
    ...
    async def _calculate_progress_trends(self, sessions: List[WorkoutSession]) -> Dict[str, Any]:
        ...
        first_avg_exercises = sum(len(s.workout_exercises) + len(s.aerobic_exercises) for s in first_half) / len(first_half)
        second_avg_exercises = sum(len(s.workout_exercises) + len(s.aerobic_exercises) for s in second_half) / len(second_half)

        # Determine trends
        volume_trend = "increasing" if second_avg_volume > first_avg_volume * 1.05 else \
                      "decreasing" if second_avg_volume < first_avg_volume * 0.95 else "stable"
        
        duration_trend = "increasing" if second_avg_duration > first_avg_duration * 1.05 else \
                        "decreasing" if second_avg_duration < first_avg_duration * 0.95 else "stable"
        
        exercise_trend = "increasing" if second_avg_exercises > first_avg_exercises * 1.05 else \
                        "decreasing" if second_avg_exercises < first_avg_exercises * 0.95 else "stable"

        return {
            "volume_trend": volume_trend,
            "duration_trend": duration_trend,
            "exercise_count_trend": exercise_trend,
            "volume_change_percent": round(((second_avg_volume - first_avg_volume) / first_avg_volume * 100) if first_avg_volume > 0 else 0, 1),
            "duration_change_percent": round(((second_avg_duration - first_avg_duration) / first_avg_duration * 100) if first_avg_duration > 0 else 0, 1)
        }

    async def _calculate_avg_volume(self, sessions: List[WorkoutSession]) -> float:
    ...
    async def _calculate_exercise_progress(
        ...
        total_reps = sum(we.reps * we.sets for we in workout_exercises if we.reps)
        ...
        recent_avg_weight = sum(we.weight for we in recent_workouts if we.weight) / len([we for we in recent_workouts if we.weight]) if any(we.weight for we in recent_workouts) else 0
        recent_avg_volume = sum(we.weight * we.reps * we.sets for we in recent_workouts if we.weight and we.reps) / len(recent_workouts)
        ...
        return {
            ...
            "progress": {
                ...
                "weight_range": f"{min_weight}-{max_weight} kg" if weights else "N/A",
