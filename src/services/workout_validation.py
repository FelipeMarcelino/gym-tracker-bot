...
def validate_exercise_data(
    exercise_data: Dict[str, Any], exercise_type: str = 'resistance'
) -> bool:
    """
    ...
    is_isometric = exercise_data.get(
        'exercise_type'
    ) == 'isometric' or is_isometric_exercise(exercise_name)
    ...
    if (
        'sets' not in exercise_data
        or exercise_data.get('sets') != inferred_sets
    ):
...
def get_user_friendly_error_message(errors: List[Dict[str, Any]]) -> str:
    ...
    message_parts = [
        '⚠️ **Alguns exercícios precisam de mais informações:**\n'
    ]
    ...
    if other_errors:
        ...
        for exercise, msg in other_errors:
            ...
            if 'número diferente' in msg:
                message_parts.append(
                    f'  • {exercise}: número de séries e pesos não correspondem'
                )
            else:
