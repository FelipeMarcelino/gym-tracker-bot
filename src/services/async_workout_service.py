...
class AsyncWorkoutService:
        ...
        try:
            async with get_async_session_context() as session:
                async with session.begin():
                    ...
                    await self._update_session_data_async(
                        session, workout_session, parsed_data
                    )
                    ...
                    resistance_exercises = parsed_data.get(
                    ...
                    aerobic_exercises = parsed_data.get(
                        'aerobic_exercises', []
                    )
            ...
            logger.exception(
                f'Database error adding exercises to session {session_id}'
            )
            raise DatabaseError(
    ...
    async def _process_resistance_exercises_async(
        ...
        if exercise_names:
            ...
            result = await session.execute(stmt)
            existing_exercises = {
                ex.name.lower(): ex for ex in result.scalars().all()
            }
        else:
            existing_exercises = {}
        ...
        for exercise_data in aerobic_exercises:
            ...
            if exercise_name_lower in existing_exercises:
                ...
                exercise = Exercise(
                    ...
                    muscle_group=infer_muscle_group(
                        exercise_name, ExerciseType.AEROBICO.value
                    ),
    ...
    async def get_user_session_status(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            async with get_async_session_context() as session:
                ...
                now = datetime.now()
                time_diff = now - datetime.combine(
                    last_session.date, last_session.start_time
                )
                ...
                is_active = (
                    SessionStatus.ATIVA
                    if minutes_passed < (timeout_hours * 60)
                    else SessionStatus.FINALIZADA
                )
            ...
            logger.exception(
                f'Error getting session status for user {user_id}'
            )
            raise DatabaseError(
                ...
                async with session.begin():
                    ...
                    stmt = (
                        ...
                        .options(
                            ...
                            selectinload(WorkoutSession.aerobics).selectinload(
                    ...
                    start_datetime = datetime.combine(
                        workout_session.date, workout_session.start_time
                    )
                    ...
                    if end_time < workout_session.start_time:
                        end_datetime = datetime.combine(
                            workout_session.date + timedelta(days=1), end_time
                        )
                    else:
                        end_datetime = datetime.combine(
                            workout_session.date, end_time
                        )
                    ...
                    logger.info(
                        f'Session {session_id} finished for user {user_id}'
                    )
    ...
    def _calculate_session_stats_sync(
        ...
        total_sets = sum(
            ex.sets for ex in workout_session.exercises if ex.sets
        )
        total_volume_kg = sum(
            sum(weights)
            for ex in workout_session.exercises
            if ex.weights_kg and isinstance(ex.weights_kg, list)
            for weights in [ex.weights_kg]
        )

        # Calculate aerobic stats
        cardio_minutes = sum(
            ex.duration_minutes
            for ex in workout_session.aerobics
    ...
    async def get_user_workout_analytics(
        ...
        try:
            async with get_async_session_context() as session:
                ...
                if not sessions:
                    return {
                        'message': 'No workout data found for the specified period'
                    }

                # Calculate comprehensive analytics
                return await self._calculate_comprehensive_analytics_async(
                    sessions, days
                )
    ...
    async def _calculate_comprehensive_analytics_async(
        ...
        avg_audios = (
            sum(s.audio_count for s in sessions) / total_sessions
            if total_sessions > 0
            else 0
        )
        ...
        difficulties = [
            ...
            if hasattr(ex, 'difficulty') and ex.difficulty
        ]
        avg_difficulty = (
        ...
        if days >= 7:
            ...
            frequency_per_week = (
                unique_workout_days * 7 / days if days > 0 else 0
            )
        ...
        for ex in all_resistance:
            if ex.exercise and ex.exercise.muscle_group:
                muscle_groups[ex.exercise.muscle_group] = (
                    muscle_groups.get(ex.exercise.muscle_group, 0) + 1
                )
        ...
        muscle_distribution = {
            muscle: {
                'count': count,
                'percentage': count / total_muscle_exercises * 100
                if total_muscle_exercises > 0
                else 0,
            }
        ...
        return {
            ...
            'workout_frequency': {
                ...
                'consistency_score': unique_workout_days / days * 100
                if days > 0
                else 0,
