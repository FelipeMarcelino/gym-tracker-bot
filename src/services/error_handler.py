...
class ErrorHandler:
    ...
    def _log_error(
        ...
        if error.error_code.value < 1200:  # General/Auth errors
            ...
            logger.warning(
                f'External service error: {error}', extra=log_context
            )
        else:  # Other errors
    ...
    async def _send_user_message(
        error: GymTrackerError, update: Update
    ) -> None:
    ...
    def _get_user_message(error: GymTrackerError) -> str:
        ...
        if isinstance(error, ValidationError):
            ...
            return messages.ERROR_LLM_PARSING.format(
                message=error.user_message
            )
            ...
            return messages.ERROR_UNEXPECTED.format(
...
async def handle_service_error(
    ...
    await ErrorHandler.handle_error(
        error, update, None, f'{service} operation'
    )
